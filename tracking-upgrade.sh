...
const ar='٠١٢٣٤٥٦٧٨٩';
const toEn=s=>String(s||'').replace(/[٠-٩]/g,d=>String(ar.indexOf(d))).replace(/[۰-۹]/g,d=>String('۰۱۲۳۴۵۶۷۸۹'.indexOf(d)));
...
document.querySelectorAll('a[data-event]').forEach(a=>a.addEventListener('click',()=>dl.push({event:a.dataset.event,link_url:a.href,landing_path:location.pathname}),{passive:true}));
document.querySelectorAll('form[data-lead-form]').forEach(form=>form.addEventListener('submit',async e=>{
 ...
 try{
   ...
   msg.className='msg ok';msg.innerHTML='تم استلام طلبك بنجاح. <a data-event="whatsapp_after_form" href="https://wa.me/966556044425?text='+encodeURIComponent('مرحباً، أرسلت طلباً عبر موقع كورت إكسبرت. رقم الطلب: '+submission_id)+'">متابعة عبر واتساب</a>';
...
for f in $(find "$ROOT" -name index.html -type f); do
  sed -i 's#src="/assets/app.js" defer#src="/assets/app.js?v='"$SHA"'" defer#g' "$f"
done
